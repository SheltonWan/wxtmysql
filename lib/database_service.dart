...
class DatabaseService {
  ...
  Future<void> _ensureDatabaseExists(String charset, String collate) async {
    ...
    try {
      ...
      adminConnection = await MySqlConnection.connect(adminSettings).timeout(Duration(seconds: 30));

      // 检查数据库是否存在
      final result = await adminConnection
          .query('SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME = ?', [_database]);

      if (result.isEmpty) {
        _logger.info('Database "$_database" does not exist, creating...');

        // 创建数据库
        await adminConnection.query('CREATE DATABASE `$_database` CHARACTER SET $charset COLLATE $collate');
      ...
      // 根据错误类型决定是否重新抛出异常
      if (e.toString().contains('Access denied') || e.toString().contains('Unknown database')) {
  ...
  /// 执行数据库操作的通用方法
  Future<T> _executeWithConnection<T>(Future<T> Function(MySqlConnection) operation, [String? operationType]) async {
  ...
  /// 执行查询
  Future<Results> query(String sql, [List<Object?>? values]) async {
    return await _executeWithConnection<Results>((conn) => conn.query(sql, values), 'Query: $sql');
  }

  /// 执行插入并返回插入的ID
  Future<int> insert(String sql, [List<Object?>? values]) async {
    final result = await _executeWithConnection<Results>((conn) => conn.query(sql, values), 'Insert: $sql');
    return result.insertId ?? 0;
  }

  /// 执行更新
  Future<int> update(String sql, [List<Object?>? values]) async {
    final result = await _executeWithConnection<Results>((conn) => conn.query(sql, values), 'Update: $sql');
    return result.affectedRows ?? 0;
  }

  /// 执行删除
  Future<int> delete(String sql, [List<Object?>? values]) async {
    final result = await _executeWithConnection<Results>((conn) => conn.query(sql, values), 'Delete: $sql');
    return result.affectedRows ?? 0;
  }
  ...
  Future<void> _markConnectionAsInvalid(PooledConnection pooledConnection) async {
    if (_connectionPool != null) {
      ...
      _logger.warning('⚠️ Connection marked as invalid due to transaction failure');
    }
  }
  ...
  Future<bool> databaseExists() async {
    try {
      ...
      final testConnection = await MySqlConnection.connect(testSettings).timeout(Duration(seconds: 10));
  ...
  Future<void> createDatabase({
    ...
    try {
      ...
      await adminConnection
          .query('CREATE DATABASE $ifNotExistsClause `$_database` CHARACTER SET $charset COLLATE $collate');
  ...
  Future<Map<String, dynamic>> getDatabaseInfo() async {
    try {
      ...
      final tablesResult = await query(
          'SELECT COUNT(*) as table_count FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = ?', [_database]);
